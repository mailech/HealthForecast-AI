...
save_dependency = Depends(RoleChecker(allowed_roles=["Doctor", "System Administrator"]))
...
def save_prediction(prediction_save: PredictionSave):
    ...
    """
    try:
        # Construct log of feature properties
        features_used = {
...
@router.get("/patient/{patient_id}", response_model=List[PredictionInDB], dependencies=[predict_dependency])
...
@router.get("", response_model=List[PredictionInDB], dependencies=[predict_dependency])
